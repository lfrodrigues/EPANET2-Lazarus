...
uses
  LCLType, LCLIntf, SysUtils, Classes, Graphics, Controls,
  Forms, Dialogs, Grids, ClipBrd, ExtCtrls, StdCtrls, Xprinter,
  Uglobals, Uutils, Printers, LResources;
...
begin
  ...
  // Draw column headings in row 0
    ...
    begin
      if vCol = 0 then
        TextOutAlign(Canvas, Rect, taLeftJustify, ColLabel[vCol])
      else
        TextOutAlign(Canvas, Rect, taCenter, ColLabel[vCol]);
    end
...
//----------------------------------------------
// Copies table to file or to Clipboard.
//----------------------------------------------
begin
