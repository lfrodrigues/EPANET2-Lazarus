...
begin
    FColHeading1 := Value;
   if ComponentsCreated then  FGrid.Cells[0,0] := FColHeading1;
end;

procedure TPropEdit.SetColHeading2(Value: String);
begin
    FColHeading2 := Value;
    if ComponentsCreated then  FGrid.Cells[1,0] := FColHeading2;
...
begin
  VisibleRows := (ClientHeight - RowHeight) div
                   (RowHeight + 1);
  with FGrid do
  begin
  ...
  //Establish column widths
    DefaultColWidth := (ClientWidth  - 2) div 2;
  end;
...
begin
  ...
  { Pass keystroke onto Grid }
    SendMessage(FGrid.Handle,WM_KEYDOWN,Key,0); //FIXME
  end;
end;
...
// Check if S is different from current value
    ...
    // Call validation procedure if it exists
      if (Assigned(FOnValidate)) then
        FOnValidate(self,FRow-1,S,Errmsg,IsValid);
    ...
    // If not valid then raise exception
         ...
         raise EInvalidProperty.Create('Invalid Property Value');
    except
...
// Set button status if row's edit style is Ellipsis
  if (FProps^[FRow-1].Style = esButton) then
    ButtonVisible := True
  else
    ButtonVisible := False;
end;
...
// for a row whose edit style is esButton
  if (Key = VK_RETURN) and
     (FProps^[aRow-1].Style = esButton) then
  try
    if (Assigned(FOnButtonClick)) then
      FOnButtonClick(self,aRow-1);
  except;
  end
...
// Pass all other keys to the parent form
  ...
  begin
    SendMessage(Parent.Handle,WM_KEYDOWN,Key,0); //FIXME
  end;
...
// then re-draw button in pressed state.
    if (aCol = 1) and (aRow = Row) and
     (FProps^[aRow-1].Style = esButton) and
     (ButtonVisible) then
    begin
...
// state and fire the OnButtonClick event.
    ...
    begin
      if (FProps^[aRow-1].Style = esButton) then
      begin
...
begin
  ...
  if (vcol = 0) or (vRow = 0) then Exit;
  with Sender as TStringGrid do
  begin
    with Canvas do
    begin

      if (FProps^[vRow-1].Style = esReadOnly) then
      begin
      ...
      if (gdSelected in State) and
         (FProps^[vRow-1].Style = esButton) then
...
begin
  ...
  //desenhar os "tres pontinhos"
  aCanvas.Brush.Color := clBlack;
  R1.Top := R.Top + H;
  R1.Bottom := R1.Top + 2;

  R1.Left := R.Left+Flags;
  R1.Right := R1.Left + 2;
  aCanvas.FillRect( R1 );

  R1.Left := R.Left+Flags-5;
  R1.Right := R1.Left + 2;
  aCanvas.FillRect( R1 );
...
// No editor used for following conditions
  ...
  if FProps^[CurRow-1].Style = esReadOnly then Exit;
  if FProps^[CurRow-1].Style = esButton then Exit;

// Activate the single line text edit control
  if (FProps^[CurRow-1].Style = esEdit) then
  begin

  // Determine max. length and edit mask
    FEdit.MaxLength := FProps^[CurRow-1].Length;
    EditMask := FProps^[CurRow-1].Mask;
  ...
  // Activate the control
    ...
    if (Key <> #13) then
       SendMessage(FEdit.Handle, LM_CHAR, Word(Key), 0);
  end

// Activate the combobox edit control
  else if (FProps^[CurRow-1].Style in [esComboList, esComboEdit]) then
  with FCombo do
  begin
    Items.Clear;
    Items.SetText(PChar(FProps^[CurRow-1].List));
    if FProps^[CurRow-1].Style = esComboList then
    begin
...
// Re-size the editor's grid and populate its cells
  with FGrid do
  begin
    RowCount := high(PropArray) - low(PropArray) + 1 +1  ; //+1 for header
    ResizeGrid(self);
    for i := 1 to RowCount - 1 do
    begin
      Cells[0,i] := PropArray[i-1].Name;
      Cells[1,i] := Values[i-1]
    end;
    Col := 1;
