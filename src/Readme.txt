...
of linear equations. EPANET can be used for all kinds of drinking water modeling: 
...
be able to run EPANET nativelly on Linux. So I started to port it from Delphi into 
Lazarus. Since my scholarship has finished and I don't the time to work on this 
I'm releasing so someone interested can continue my work.

Since Lazarus didn't support MDI forms I had to make some ajustments to the 
interface but everyone with is used to EPANET should be able to use it 
without problems. Many Lazarus bugs were fixed while porting the code, a 
Charting and i18n systems were also developed.

The are still two missing parts with I this are import:

    * Printing System - EPANET original printing system is based on Windows 
	Metafile with is not supported on any other platform, I've made a 
	Windows component to ease the port but it needs to be replaced with 
	cross platform code. I'vs started to port a Delphi Print/Print Preview 
	component to Lazarus with is complelly cross platform but didn't had a 
	chance to finish and integrate it.
    * I18n - To translate EPANET into other languages the Lazarus i18n system 
	should be used. I've made some patched with are now part of Lazarus 
...
Anyone interest in finishing this project please contact me to: 
lfrodrigues at gmail dot com
