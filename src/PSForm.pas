...
Windows, LCLIntf, LCLType, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  Printers, StdCtrls, ExtCtrls, ComCtrls, Math, PgSetup, LResources,
  PrintersDlgs, OSPrinters;
...
begin
  ...
  xPixPerInch := GetDeviceCaps(TWinPrinter(Printer).Handle,LOGPIXELSX);
  yPixPerInch := GetDeviceCaps(TWinPrinter(Printer).Handle,LOGPIXELSY);
  physWidth := GetDeviceCaps(TWinPrinter(Printer).Handle,PHYSICALWIDTH);
  physHeight := GetDeviceCaps(TWinPrinter(Printer).Handle,PHYSICALHEIGHT);
  physOffsetX := GetDeviceCaps(TWinPrinter(Printer).Handle,PHYSICALOFFSETX);
  physOffsetY := GetDeviceCaps(TWinPrinter(Printer).Handle,PHYSICALOFFSETY);
  PrintWidth := GetDeviceCaps(TWinPrinter(Printer).Handle,HORZRES);
  PrintHeight := GetDeviceCaps(TWinPrinter(Printer).Handle,VERTRES);
