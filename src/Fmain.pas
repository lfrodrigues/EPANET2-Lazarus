...
{                                                                   }
{   EPANET2W is the GUI version of EPANET 2 -- a program that   }
...
uses
  ...
  Printers, PgSetup,  Xprinter, Uglobals,
  Uutils, ExtDlgs, LResources;
...
var
  i  : Integer;
begin
  Height := 74; //Just show one of the bottom panels
  Constraints.MaxHeight := Height;
  Constraints.MinHeight := Height;
// Load custom cursors
  Screen.Cursors[crXHAIR]   := LoadCursorFromLazarusResource('xhair');
  Screen.Cursors[crZOOMIN]  := LoadCursorFromLazarusResource('zoomin');
  Screen.Cursors[crZOOMOUT] := LoadCursorFromLazarusResource('zoomout');
  Screen.Cursors[crFIST]    := LoadCursorFromLazarusResource('fist');
  Screen.Cursors[crMOVE]    := LoadCursorFromLazarusResource('move');
  Screen.Cursors[crPENCIL]  := LoadCursorFromLazarusResource('pencil');
  Screen.Cursors[crARROWTIP]:= LoadCursorFromLazarusResource('arrowtip');
...
//create page setup
  PageSetupDialog:= TPageSetupDialog.Create(Self);

// Disable printing options if there are no printers
 //create thePrinter object
  thePrinter := TPrintControl.Create(MainForm);
  thePrinter.name := 'thePrinter';
...
//
  ...
  // Create Browser & Map child forms
    ...
    //resize and arranje forms
    MnuArrangeClick(Self);
...
// Enable/disable printing (if printing is allowed)
  ...
  begin
    MnuPrint.Enabled :=
      (LastActiveForm is TStatusForm) or
      (LastActiveForm is TEnergyForm) or
      (LastActiveForm is TCalibReportForm) or
      (LastActiveForm is TTableForm) or
      (LastActiveForm is TGraphForm) or
      (LastActiveForm is TContourForm) or
      (LastActiveForm is TMapForm) or
      ((LastActiveForm is TBrowserForm) and
...
//------------------------------------------------------
// Saves network map to metafile or AutoCad DXF
...
//-------------------------------------------------------------
begin
    if LastActiveForm is TMapForm then
      TMapForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TGraphForm then
      TGraphForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TTableForm then
      TTableForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TStatusForm then
      TStatusForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TContourForm then
      TContourForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TCalibReportForm then
      TCalibReportForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TEnergyForm then
      TEnergyForm(LastActiveForm).Print(Dest)
    else if LastActiveForm is TBrowserForm then
      TBrowserForm(LastActiveForm).Print(Dest);
end;
...
// Can only copy graph, table, or report forms
  MnuCopy.Enabled := (LastActiveForm is TMapForm) or
                     (LastActiveForm is TGraphForm) or
                     (LastActiveForm is TTableForm) or
                     (LastActiveForm is TContourForm) or
                     (LastActiveForm is TStatusForm) or
                     (LastActiveForm is TCalibReportForm) or
                     (LastActiveForm is TEnergyForm);
end;
...
begin
  if LastActiveForm is TMapForm then
    TMapForm(LastActiveForm).CopyTo
  else if LastActiveForm is TGraphForm then
    TGraphForm(LastActiveForm).CopyTo
  else if LastActiveForm is TTableForm then
    TTableForm(LastActiveForm).CopyTo
  else if LastActiveForm is TContourForm then
    TContourForm(LastActiveForm).CopyTo
  else if LastActiveForm is TStatusForm then
    TStatusForm(LastActiveForm).CopyTo
  else if LastActiveForm is TCalibReportForm then
    TCalibReportForm(LastActiveForm).CopyTo
  else if LastActiveForm is TEnergyForm then
    TEnergyForm(LastActiveForm).CopyTo;
end;
...
// Options menu item is enabled depending on type of active window
  MnuReportOptions.Enabled :=
    ...
    (LastActiveForm is TTableForm) or
    (LastActiveForm is TGraphForm) or
    (LastActiveForm is TContourForm);
end;
...
//---------------------------------------------------------
begin
 if (LastActiveForm is TContourForm) then
    with LastActiveForm as TContourForm do SetContourOptions
  else if (LastActiveForm is TTableForm) then
    with LastActiveForm as TTableForm do SetTableOptions
  else if (LastActiveForm is TCalibReportForm) then
    with LastActiveForm as TCalibReportForm do SetCalibOptions;
end;
...
//--------------------------------------------------------
// Arranges child windows when Window|Arrange selected
//--------------------------------------------------------
begin
//set o fill screen
  MapForm.Left := 0;
  MapForm.Top := MainForm.Height + 26;
  MapForm.Width := Screen.Width - BrowserForm.Width - 10;
  MapForm.Height := Screen.Height - MainForm.Height - 82;

  BrowserForm.Left := Screen.Width - BrowserForm.Width - 4;
  BrowserForm.Top := MainForm.Height + 26;
end;
...
begin
   OpenFile(Sender,'C:\Programas\Epanet2P\Exemplos\net3.net');
end;
...
begin
   with TEnergyForm.Create(self) do show;
end;
...
//-----------------------------------------------------
begin //TODO fix...
  TempInputFile  := Uutils.GetTempFile(TempDir,'en1');
  TempReportFile := Uutils.GetTempFile(TempDir,'en2');
  TempOutputFile := Uutils.GetTempFile(TempDir,'en3');
end;
...
// Display any warning messages in Status Report
  ...
  begin
    ...
    if LastActiveForm is TStatusForm then
      TStatusForm(LastActiveForm).SelectText(TXT_WARNING);
  end;
end;
...
begin
  ...
  //TODO: applicatioon help command
