...
uses
  LCLType, LCLIntf, SysUtils, Messages, Classes, Graphics, Controls,
  Forms, Dialogs, StdCtrls, Buttons, Grids, ExtCtrls,
  Uglobals, Uutils, TAGraph, TASeries, LResources;
...
// Make PatternID edit box the active control
  ...
  TASerie := TSerie.Create(TAChart1);
  TASerie.SeriesColor := clBlue;
  TAArea  := TAreaSeries.Create(TAChart1);
  TAArea.SeriesColor := $0080ff;
  TAArea.Stairs := True;
  TAArea.InvertedStairs := False;
  TAChart1.AddSerie(TAArea);
  TAChart1.AddSerie(TASerie);
...
// Include length of time period in X-axis title
  ...
  TAChart1.BottomAxis.Title.Caption := TXT_AXIS_TITLE + PatTstep + TXT_HRS;
  TAChart1.LeftAxis.Title.Caption := TXT_MULTIPLIER;
...
// Display line for average multiplier value
  if TAArea.Count >= 2 then with TASerie do
  begin
    Avg := Avg/(TAArea.Count-1);
    AddXY(0, Avg, '', clTAColor);
    AddXY(T, Avg, '', clTAColor);
    TAChart1.LeftAxis.Title.Caption := Format(FMT_LEFT_AXIS,[Avg]);
  end;
...
procedure TPatternForm.FormClose(Sender: TObject;
