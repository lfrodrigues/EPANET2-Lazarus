...
uses
  ...
  StdCtrls, ExtCtrls,  Grids, Math, Uglobals, Uutils, TAGraph, TASeries,
  LResources;
...
type
  TCurveForm = class(TForm)
    ...
    procedure CurveTypeChange(Sender: TObject);
  private
...
// Add curve types to CurveType combobox
  ...
  TAPoint := TSerie.Create(TAChart1);
  TALine:= TSerie.Create(TAChart1);

  TAChart1.AddSerie(TALine);
  TAChart1.AddSerie(TAPoint);

  TAPoint.ShowPoints := True;
  TAPoint.ShowLines := False;
  TAPoint.SeriesColor := clLime;
  TAPoint.Pointer.Style :=  psRectangle;

  TALine.ShowPoints := False;
  TALine.ShowLines := True;
  TALine.SeriesColor := clRed;
  TALine.Pointer.Style :=  psRectangle;
end;
...
// Label chart's axes
  ...
  TAChart1.BottomAxis.Title.Caption := Xlabel[I] + Xunits[I];
  TAChart1.LeftAxis.Title.Caption := Ylabel[I] + Yunits[I];
...
// Draw curve & data points (except for fitted pump curve)
  ...
  begin
    ...
    for I := 1 to N do begin
       TALine.AddXY(X[I], Y[I], '', clTAColor); //FIXME
    end;
    if IsPumpCurve then begin
//        Pointer.Visible := False;
        TALine.ShowPoints := false;
    end else begin
//        Pointer.Visible := True;
        TALine.ShowPoints := True;
    end;
    Active := True;
    TALine.Active := True;
  end;
...
// Display original pump curve data points on fitted pump curve
  if IsPumpCurve then with TAPoint do
  begin
    Active := False;
    N := GetData;
    for I := 1 to N do begin
      AddXY(X[I], Y[I], '', clTAColor);
    end;
    Active := True;
  end;
end;
...
procedure TCurveForm.CurveTypeChange(Sender: TObject);
//-------------------------------------------------------
