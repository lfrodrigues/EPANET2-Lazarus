...
uses
  LCLType, SysUtils, LCLIntf, Messages, Classes, Graphics, Controls,
  Forms, StdCtrls, Printers, ExtCtrls, Buttons, Dialogs, TAGraph, XPForm, mymetafile,
  LResources, OSPrinters;
...
type
  ...
  TPrintPage = TLRMetafile;            { Each page stored in a metafile }
  TPrintCanvas = TLRMetafileCanvas;    { Metafile canvas }
         ...
         try
            ...
            while not fCancelPrinting and (i <= PageCount) do
               ...
               if fShowProgress and Assigned(fProgressForm) then with fProgressForm do
                  ...
                  Label1.Caption := Format('A imprimir página %d of %d',[i, PageCount]);
                  Update;
                  end;
...
procedure TPrintControl.DisplayPage(Page : Integer);
   ...
   if (Page > 0) AND (Page <= PageCount) then
      ...
      if fPreviewForm.ShwGrdBtn.Down then
        ...
        fPreviewForm.Panel2.Caption := Format('Página %d of %d',
                                              [Page, PageCount]);
...
procedure TPrintControl.CalculateMeasurements;
   ...
   { Calculate the number of pixels per inch vertical and horizontal.
     'GetDeviceCaps' is a API call. }
   PixelsPerInchVertical := GetDeviceCaps( TWinPrinter(Printer).Handle,LOGPIXELSY );
   PixelsPerInchHorizontal := GetDeviceCaps( TWinPrinter(Printer).Handle,LOGPIXELSX );

   { Get the physical size of the page. }
   TotalPageWidthPixels := GetDeviceCaps(TWinPrinter(Printer).Handle, PHYSICALWIDTH);
   TotalPageHeightPixels := GetDeviceCaps(TWinPrinter(Printer).Handle, PHYSICALHEIGHT);

   { Get the printable size of the page. }
   PageWidthPixels := GetDeviceCaps(TWinPrinter(Printer).Handle, HORZRES);
   PageHeightPixels := GetDeviceCaps(TWinPrinter(Printer).Handle, VERTRES);

   { Get the gutters on the left, top, right, and bottom. }
   GutterLeft := GetDeviceCaps(TWinPrinter(Printer).Handle, PHYSICALOFFSETX);
   GutterTop  := GetDeviceCaps(TWinPrinter(Printer).Handle, PHYSICALOFFSETY);
