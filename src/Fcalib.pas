...
uses
  ...
  Clipbrd, Xprinter, Uglobals, Uutils, TAGraph, TASeries, LResources;
...
//  CalibReportForm: TCalibReportForm;
   ComputedSeries, ObservedSeries: TBarSeries;
...
//---------------------------------------------------
begin
  ComputedSeries := TBarSeries.Create(TAChart2);
  ComputedSeries.Title := 'Computed';
  ComputedSeries.SeriesColor := clRed;
  ObservedSeries := TBarSeries.Create(TAChart2);
  ObservedSeries.Title := 'Observed';
  ObservedSeries.BarBrush.Style := bsDiagCross;
  ObservedSeries.BarBrush.Color := clGreen;
  TAChart2.AddSerie(ComputedSeries);
  TAChart2.AddSerie(ObservedSeries);
  TAChart2.Legend.Visible := True;
...
begin
  ...
  begin
    if BoldFonts then
      ...
      Title.Font.Style := [];
//TODO    LabelsFont.Assign(Title.Font);
  end;
end;
...
begin
  ...
  c := TAChart2.Series.Count;
  for i := 0 to c -1 do begin
      TChartSeries(TAChart2.Series[0]).Free;
      TAChart2.Series.Delete(0);
  end;
...
//If there are any data, then
  ...
  //Add obs. & simul. means to Mean Comparison chart
    ComputedSeries.Add(stats[3],ID,clTAColor);
    ObservedSeries.Add(stats[2],ID,clTAColor);
  end;
end;
...
//-----------------------------------------------------
procedure getMaxMinVal(TAChart: TChart; var MinX, MaxX, MinY, MaxY: Double);
var i,j: integer;
    c: PChartCoord;
begin
     MinX:=0; MaxX:=0; MinY:=0; MaxY:=0;
     for i := 0 to TAChart.SeriesCount-1 do begin
        for j := 0 to TChartSeries(TAChart.Series[i]).Count -1 do begin
           c := TChartSeries(TAChart.Series[i]).Coord[i];
...
begin
  ...
  //Find coordinates of perfect correlation line
    ...
    getMaxMinVal(TAChart1, x1, x2, y1, y2);
  ...
  //Create FastLineSeries for perfect correlation line
    bSeries := TSerie.Create(TAChart1);
    TAChart1.AddSerie(bSeries);
    with bSeries do
    try
      ParentChart := TAChart1;
      Title := '';
      ShowInLegend := False;
      SeriesColor := clBlack;
      ShowPoints := false;
      AddXY((x1-z1),(y1-z2),'',clTAColor);
      AddXY((x2+z1),(y2+z2),'',clTAColor);
    finally
    end;
    Visible := True;
  end;
          ...
          //Check if new location belongs in the report
            ...
            //Create new PointSeries for correlation plot
              ...
              try
                ...
                ShowLines := false;
                ShowPoints := true;
                Pointer.Style := TASeries.psCross;
                Pointer.Pen.Width := 2;
...
//and update error statistics
  ...
  begin
    with TAChart1 do
      TChartSeries(Series[SeriesCount-1]).AddXY(v, v1, '', clTAColor);
...
var
  theChart: TChart;
begin
{FIXME // Determine which chart to copy
  if PageControl1.ActivePage = Tabsheet2 then
    theChart := Chart1
  else
    theChart := Chart2;
...
// Create a stringlist to hold the data
  ...
  // Add data from Chart1 (correlation plot) to list
    else if PageControl1.ActivePage = TabSheet2 then with TAChart1 do
    begin
      ...
      begin
        ...
        begin
          for J := 0 to Count-1 do
            Slist.Add(Title + #9 + FloatToStr(PChartCoord(Coord[j])^.x) + #9 +
               FloatToStr(PChartCoord(Coord[j])^.y));
        end;
      end;
    end

  // Add data from Chart2 (mean comparisons) to list
    else if PageControl1.ActivePage = TabSheet3 then with TAChart2 do
    begin
      Slist.Add(Title.Text[0]);
      Slist.Add('Location' + #9 + 'Computed' + #9 + 'Observed');
      for J := 0 to TChartSeries(Series[0]).Count-1 do
        Slist.Add(PChartCoord(TChartSeries(Series[0]).Coord[J])^.text + #9 +
                  FloatToStr(PChartCoord(TChartSeries(Series[0]).Coord[J])^.y) + #9 +
                  FloatToStr(PChartCoord(TChartSeries(Series[1]).Coord[J])^.y));
    end;
