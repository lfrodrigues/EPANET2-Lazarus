...
uses Windows, SysUtils, LCLIntf, Math, Classes, Dialogs, TAGraph, Graphics;
...
procedure TextOutAlign(Canvas: TCanvas; rect: TRect; align: TAlignment; text: String);
...
begin                   
  try
    ...
    tmpR:=aChart.GetRectangle;
    tmpBitmap.Width := tmpR.Right-tmpR.Left;
    tmpBitmap.Height:= tmpR.Bottom-tmpR.Top;
    tmpBitmap.Canvas.CopyRect(tmpR, aChart.Canvas, tmpR);
...
//    if Source < 0 then raise EFOpenError.CreateFmt(SFOpenError, [FileName]);
    if Source < 0 then raise EFOpenError.Create('ERROR no filename'+FileName);
    try
      Dest := FileCreate(Destination); { create output file; overwrite existing }
      if Dest < 0 then raise EFOpenError.Create('ERROR no filename'+Destination);
...
//this replaces canvas.textrect so that #13
procedure TextOutAlign(Canvas: TCanvas; Rect: TRect; align: TAlignment; text: String);
...
begin
  c := 0;
  for i:= 0 to length(text)-1 do if text[i] = #13 then inc(c);
  if c > 0 then begin
    SetLength(lines, c+1);
      for i := 0 to c-1 do begin
        lines[i] := copy(text, 0, pos(#13, text)-1);
        Text := copy(text, pos(#13, text) +1 , length(text) - pos(#13, text));
     end;
     lines[c] := Text;
  end else begin
    SetLength(lines, 1);
  ...
  for i:=0 to length(lines)-1 do begin
    case align of
      ...
      taRightJustify: begin
        x:= Rect.Right - Canvas.TextWidth(lines[i])-2;
      end;
      taCenter:begin
        x:= Rect.Left + ( (Rect.Right - Rect.Left) div 2 )
                       - (Canvas.TextWidth(lines[i]) div 2);
      end;
    end;
  Canvas.TextOut(x, y, lines[i]);
  y := Canvas.TextHeight( lines[i] );
  end;
end;
