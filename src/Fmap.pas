...
uses
  LCLIntf, LCLType, SysUtils, Messages, Classes, Graphics, Controls,
  Forms, Dialogs, Spin, StdCtrls, ExtCtrls, Buttons, Menus,
{*** Updated 12/29/00 ***}
  Clipbrd, Math, Printers, Xprinter, Uglobals, Uutils, Umap, LResources, MyMetafile;
...
type
  ...
  private
    ...
    procedure DrawToMetafile(aMetafile: TLRMetafile);
...
var i, j: Integer;
begin
  //poor man's replacement to InvertRect
  for i:=0 to HiliteBitmap.Width do begin
    for j:=0 to HiliteBitmap.Height do begin
      HiliteBitmap.Canvas.Pixels[i,j] := $FFFFFF - HiliteBitmap.Canvas.Pixels[i,j];
    end;
  end;
  self.Canvas.CopyRect(HiliteRect,HiliteBitmap.Canvas,Rect(0,0,HiliteBitmap.Width,HiliteBitmap.Height));
end;
...
begin
  ...
  try
    ...
    tmpR.Top := 0;
    tmpR.Left := 0;
    tmpR.Right := ClientWidth;
    tmpR.Bottom := ClientHeight;
    aBitmap.Width := ClientWidth;
    aBitmap.Height := ClientHeight;
    aBitmap.Canvas.Draw(0,0, Map.Bitmap);
...
//-------------------------------------------------------
var
  MyMetafile: TLRMetafile;
begin
  MyMetafile := TLRMetaFile.Create;   // Create the metafile
  try
...
procedure TMapForm.DrawToMetafile(aMetafile: TLRMetafile);
//------------------------------------------------------
// Draws network map on aMetafile.
//------------------------------------------------------
var
  aMetafileCanvas: TLRMetafileCanvas;
...
//Create a canvas for the metafile & a clipping region for the canvas
  aMetafileCanvas := TLRMetafileCanvas.Create(aMetafile,0);
